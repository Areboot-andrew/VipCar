...
- `/admin/cms` переписано з сирого списку ключів у структурований редактор: Головна, Блоки, Контакти, SEO, Технічні. Логіка взята з референсу `energy-`, але адаптована під CarCRM-тему, з preview двоколірних заголовків, бібліотекою іконок і зрозумілим конструктором блоків.
...
- Demo seed v2 додає 3 водіїв, 3 клієнтів і 5 тестових бронювань зі статусами, інвойсами, чатами та фінансовим breakdown для перевірки календаря, заявок, водійського кабінету і калькуляторної логіки. Для чистого тестового старту є `RESET_DEMO_DATA=true`.
